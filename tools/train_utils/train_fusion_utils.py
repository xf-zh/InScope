...
def train_one_epoch(model, optimizer, train_loader1, train_loader2, model_func, lr_scheduler, accumulated_iter, optim_cfg,
                    rank, tbar, total_it_each_epoch, dataloader_iter1, dataloader_iter2, tb_log=None, leave_pbar=False,
                    ...
                    use_amp=False):
    if total_it_each_epoch == len(train_loader1):
        dataloader_iter1 = iter(train_loader1)
        dataloader_iter2 = iter(train_loader2)
    ...
    for cur_it in range(start_it, total_it_each_epoch):
        try:
            ...
            batch2 = next(dataloader_iter2)
            print('new iters')
        # print('batch1 frame_id:', batch1['frame_id'])
        # print('batch2 frame_id:', batch2['frame_id'])
        # 判断batch输出的数据是否一致
        if batch1['frame_id'][0] != batch2['frame_id'][0]:
            count_iy += 1
            print('batch1 frame_id:', batch1['frame_id'])
            print('batch2 frame_id:', batch2['frame_id'])
            raise ValueError('batch1 and batch2 frame_id is not equal')
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            loss, tb_dict, disp_dict = model_func(model, batch1, batch2)
        ...
        if rank == 0:
            batch_size = batch1.get('batch_size', None)
    ...
    # print('count_iy:', count_iy)
    return accumulated_iter


def train_model(model, optimizer, train_loader1, train_loader2, model_func, lr_scheduler, optim_cfg,
                start_epoch, total_epochs, start_iter, rank, tb_log, ckpt_save_dir, train_sampler1=None,train_sampler2=None,
    ...
    with tqdm.trange(start_epoch, total_epochs, desc='epochs', dynamic_ncols=True, leave=(rank == 0)) as tbar:
        total_it_each_epoch = len(train_loader1)
        ...
        for cur_epoch in tbar:
            if train_sampler1 is not None:
                train_sampler1.set_epoch(cur_epoch)
            if train_sampler2 is not None:
                train_sampler2.set_epoch(cur_epoch)
            # train one epoch
            ...
            accumulated_iter = train_one_epoch(
                model, optimizer, train_loader1, train_loader2,model_func,
                lr_scheduler=cur_scheduler,
                accumulated_iter=accumulated_iter, optim_cfg=optim_cfg,
                rank=rank, tbar=tbar, tb_log=tb_log,
                leave_pbar=(cur_epoch + 1 == total_epochs),
                total_it_each_epoch=total_it_each_epoch,
                dataloader_iter1=dataloader_iter1,
                dataloader_iter2=dataloader_iter2,
