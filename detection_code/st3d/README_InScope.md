# InScope transfer learning Benchmark
This is a reproduced benchmark about transfer learning for 3D object detection on the [InScope](我们的链接) dataset.
...
Take Source Only model with Pointrcnn-IoU on InScope -> KITTI  as an example:
```shell script
sh scripts/dist_train.sh ${NUM_GPUS} --cfg_file cfgs/da-inscope-kitti_models/pointrcnniou_origin.yaml \
...
```shell script
sh scripts/dist_train.sh ${NUM_GPUS} --cfg_file cfgs/da-inscope-kitti_models/pointrcnniou_st3d++.yaml \
...
And we follow this [issue](https://github.com/CVMI-Lab/ST3D/issues/35) to use ST3D for InScope dataset.