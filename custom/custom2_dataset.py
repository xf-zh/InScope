...
class CustomDataset(DatasetTemplate):
    # init函数在实例化Customdataest对象时运行，初始化点云和对应的label文件以及目录
    ...
    # 这个函数从给定索引的数据集中加载点云样本和对应的标签数据将他们组合成dict，并返回相应的dict
    def __getitem__(self, index):
        # if self._merge_all_iters_to_one_epoch:
        #     index = index % len(self.custom_infos)
        #
        # info = copy.deepcopy(self.custom_infos[index])
        # sample_idx = info['point_cloud']['lidar_idx']#索引
        ...
        #
        # return data_dict
        sample_idx = Path(self.sample_file_list[index]).stem  # stem返回文件名
        # 进行点云数据转化，固定为4列x y z intensity
        points = np.fromfile(self.sample_file_list[index],dtype=np.float32).reshape(-1,4)[:,:4]
        # 在加载点云和标签的同时，将坐标转化成统一坐标系下
        # 得到点云标签数据
        #print(self.samplelabel_file_list[index])
        #points_label = np.loadtxt(self.samplelabel_file_list[index]).reshape(-1, 10)
        with open(self.sample_file_list[index],'r',encoding='utf-8') as infile:
            points_label = []
            for line in infile:
                data_line = line.strip("\n").split()  # 去除首尾换行符，并按空格划分
                print(data_line)
                points_label.append(np.array(data_line[1:10]))

        # 将格式进行转化为x y z dx dy dz heading
        points_label = points_label.reshape(-1,9)
        points_label = points_label[:,[4,5,6,1,2,3,8]]
        print(points_label)
        gt_names = np.array(['custom']*points_label.shape[0])
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_custom_infos':
        ...
        create_custom_infos(
            ...
            class_names=['Car', 'Pedestrian', 'Cyclist', "Cart"],
